...
Node.prototype.askOnz = function(method, endpoint, action, misc, self) {
		...
		res.on('data', function (chunk) {
  		...
  		res.on('end', function () {
       		...
       		if (action == 'check') {
       			...
       			self.info.coinbase = "Onz " + jsonObject["version"] + " Build:(" +jsonObject["build"]+")";
				self.info.node = "Onz " + jsonObject["version"] + " Build:(" +jsonObject["build"]+")";
				...
				var peer_count = jsonObject["connected"];
				self.stats.active = true;
				self.stats.peers = peer_count;
				console.info("Peer Count:", peer_count);
			} else if (action == 'forging') {
				if (jsonObject["enabled"]) {
					self.stats.hashrate = 1;
					self.stats.mining = true;
					console.info('==>', 'Forging enabled');
				} else {
					self.stats.hashrate = 0;
					self.stats.mining = false;
					console.info('==>', 'Forging disabled');
				}
       		} else if (action == 'delegates'){
				var delegateCount = jsonObject["count"];
				self.stats.gasPrice = delegateCount;
...
Node.prototype.startConnection = function() {
	console.info('Starting connection to Onz Node');
...
Node.prototype.checkConnection = function() {
	var self = this;
	self.askOnz('GET','/api/loader/status/sync','check', '', self);
	if(isConnected) {
...
Node.prototype.startSocketConnection = function() {
	if( !this._socket ){
		...
		socket = new Socket( process.env.WS_SERVER || 'ws://stats.onzcoin.com:3000');
		this.setupSockets();
	}
}
...
Node.prototype.getInfo = function() {
	...
	self.askOnz('GET','/api/peers/version','nodeinfo', '', self);
	return true;
}
...
Node.prototype.getLatestBlock = function () {
	...
	self.askOnz('GET','/api/blocks?limit=1','lastblock', timeString, self);
}
...
Node.prototype.getStats = function(forced) {
	...
	if (lastFetchAgo >= UPDATE_INTERVAL || forced === true) {
		...
		self.getPending();
		self.askOnz('GET','/api/delegates/count','delegates', '', self);
		self.askOnz('GET','/api/delegates/forging/status','forging', '', self);
		self.askOnz('GET','/api/peers/count','peers', '', self);
		self.getLatestBlock();
		self.getInfo();
...
Node.prototype.getPending = function() {
	...
	self.askOnz('GET','/api/transactions/unconfirmed','pendingtx', now, self);
	return false;	
}
...
Node.prototype.setWatches = function() {
	...
	self.askOnz('GET','/api/loader/status/sync','syncing', '', self);
}
